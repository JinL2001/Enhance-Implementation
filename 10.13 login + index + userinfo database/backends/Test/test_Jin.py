...
openai.api_key = "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"
...
conversation_history = [
    {
        "role": "system", 
        "content": "Your initial system message here..."
    }
    # Make sure all other messages in the history are also simple dictionaries.
]
...
load_knowledge_base(r'C:\Users\david\VSCode\backends\Test\Jin.txt')
...
def supplement_response_with_knowledge(user_input, conversation_history):
    ...
    chat = ChatOpenAI(model_name="gpt-4", temperature=0.3)  # Adjust if using GPT-4
    response = chat.generate(prompt, conversation_history)  # Ensure your chat.generate() method accepts a prompt

    return response.content

def generate_response_with_openai(user_input, conversation_history):
    try:
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",  # Change to "gpt-4" if you're using that model
...
def send_message():
    try:
        ...
        # Append the user message to the history as a plain dictionary
        conversation_history.append({"role": "user", "content": user_input})

        # Generate the AI response using the OpenAI API
        ai_response = generate_response_with_openai(user_input, conversation_history)

        # Make sure to check that ai_response is not empty or None
        if not ai_response:
            raise ValueError('The AI response is empty.')

        # Append the AI's response to the history as a plain dictionary
